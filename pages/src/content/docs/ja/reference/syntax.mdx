...
Purusには同じJavaScriptに出力される2種類の文字列構文があります。
...
`//;...;//` が**推奨**の構文です。`;` をデリミタとすることで `[expr]` 補完との概念的な重複を避け、URLなど `//` を含む内容でも視覚的な混乱が生じません。
...
`///...///` は完全にサポートされており、同一の出力を生成します。元々の構文で引き続き有効です。好みに応じて使用できます。

:::note
このドキュメントのコード例はすべて `//;...;//` を使用しています。どちらの形式も互換です。
:::
