...
| `blank` | `_` | Wildcard — ignored parameter or catch-all match arm |
...
| Syntax | Description |
|---|---|
| `//;text;//` | String literal — **recommended** |
| `///text///` | String literal — also valid (original syntax) |
| `//;text [expr] text;//` | Interpolated string (recommended form) |
| `///text [expr] text///` | Interpolated string (original form) |

Both syntaxes produce identical JavaScript output. `//;...;//` is preferred for readability: the `;` delimiter does not conflict with `[expr]` interpolation, and `//` in content (e.g. URLs) is never confused with the closing `;//`.
