...
Purus has two string syntaxes that compile to identical JavaScript output.
...
const tmpl be //;Hello, [name]!;//
```

`//;...;//` is the **recommended** syntax. `;` as the delimiter does not conflict with `[expr]` interpolation and avoids visual confusion when content contains `//` (such as URLs).
...
`///...///` is fully supported and produces identical output. It is the original syntax and remains valid — use it if you prefer.

:::note
All examples in this documentation use `//;...;//`. Both forms are interchangeable.
:::
...
| `\]` | `]` (literal bracket) |
| `\;` | `;` (literal semicolon, prevents early `;//` close in `//;...;//`) |
